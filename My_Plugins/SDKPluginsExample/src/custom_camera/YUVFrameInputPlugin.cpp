...
extern "C" JNIEXPORT void JNICALL
Java_com_example_danielphillips_a3020androidarnative_ArActivity_notifyNewCameraFrameN21(JNIEnv* env, jobject obj, jbyteArray cameraFrame)
{
    int frameSize = -1;
...
extern "C" JNIEXPORT void JNICALL
Java_com_example_danielphillips_a3020androidarnative_ArActivity_setDefaultDeviceOrientationLandscape(JNIEnv* env, jobject obj, jboolean isLandscape)
{
    YUVFrameInputPlugin::instance->setDefaultOrientationLandscape(isLandscape);
}

extern "C" JNIEXPORT void JNICALL
Java_com_example_danielphillips_a3020androidarnative_ArActivity_setImageSensorRotation(JNIEnv* env, jobject obj, jint rotation)
{
    YUVFrameInputPlugin::instance->setImageSensorRotation(rotation);
}

extern "C" JNIEXPORT void JNICALL
Java_com_example_danielphillips_a3020androidarnative_ArActivity_setCameraFieldOfView(JNIEnv* env, jobject obj, jdouble fieldOfView)
{
...
extern "C" JNIEXPORT void JNICALL
Java_com_example_danielphillips_a3020androidarnative_ArActivity_setFrameSize(JNIEnv* env, jobject obj, jint frameWidth, jint frameHeight)
{
...
YUVFrameInputPlugin::~YUVFrameInputPlugin()
{
    ...
    vm.env->DeleteGlobalRef(ArActivity);
}
...
void YUVFrameInputPlugin::update(const wikitude::sdk::RecognizedTargetsBucket& recognizedTargetsBucket_) {
    if ( !_jniInitialized ) {
        JavaVMResource vm(pluginJavaVM);
        jclass ArActivityClass = vm.env->FindClass("com/example/danielphillips/a3020androidarnative/ArActivity");

        _pluginInitializedMethodId = vm.env->GetMethodID(ArActivityClass, "onInputPluginInitialized", "()V");
        _pluginPausedMethodId = vm.env->GetMethodID(ArActivityClass, "onInputPluginPaused", "()V");
        _pluginResumedMethodId = vm.env->GetMethodID(ArActivityClass, "onInputPluginResumed", "()V");
        _pluginDestroyedMethodId = vm.env->GetMethodID(ArActivityClass, "onInputPluginDestroyed", "()V");
    ...
    { // mutex auto release scope
        ...
        _currentlyRecognizedTargets = std::list<wikitude::sdk::ImageTarget>(recognizedTargetsBucket_.getImageTargets());
    }
}
...
void YUVFrameInputPlugin::callInitializedJNIMethod(jmethodID methodId_) {
    if ( _jniInitialized ) {
        JavaVMResource vm(pluginJavaVM);
        vm.env->CallVoidMethod(ArActivity, methodId_);
    }
}
...
void YUVFrameInputPlugin::render() {
    ...
    std::unique_lock<std::mutex> lock(_currentlyRecognizedTargetsMutex);
        ...
        if (!_currentlyRecognizedTargets.empty()) {
            ...
            const wikitude::sdk::ImageTarget targetToDraw = _currentlyRecognizedTargets.front();
            ...
            const float TARGET_HEIGHT_IN_PIXELS = 1.0f;
